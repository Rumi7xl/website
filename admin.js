...
async function loadStats() {
  try {
    ...
    document.getElementById("totalAnnouncements").innerText = annSnap.size;
  } catch (e) {
...
if (saveIceriklerBtn) {
  saveIceriklerBtn.onclick = async () => {
    try {
      ...
      const isLive = isLiveEl ? isLiveEl.checked : false;
      const liveGame = liveGameEl ? liveGameEl.value.trim() : "";
      const liveViewers = liveViewersEl ? liveViewersEl.value.trim() : "0";
      const kickLink = kickLinkEl ? kickLinkEl.value.trim() : "";
      const kickDesc = kickDescEl ? kickDescEl.value.trim() : "";
      const tiktokLink = tiktokLinkEl ? tiktokLinkEl.value.trim() : "";
      const tiktokDesc = tiktokDescEl ? tiktokDescEl.value.trim() : "";
